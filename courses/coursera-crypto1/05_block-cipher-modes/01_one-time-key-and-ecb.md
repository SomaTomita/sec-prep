...
ブロック暗号は 1 ブロックしか変換できない。任意長へ広げるのが**利用モード (mode of operation)**。まず鍵を 1 メッセージにしか使わない**ワンタイム鍵**を扱い、素朴な ECB がなぜ壊れているかを見る。

**この1ページで分かること**

- Switching Lemma: `|X|` が十分大きければ PRP をそのまま PRF として使ってよい
- ECB は同じ平文ブロック → 同じ暗号文ブロック。1 メッセージでも意味論的安全性が崩れる
- 決定的 CTR は PRF でパッドを作って XOR。ワンタイム鍵なら安全、2 回使うと two-time pad

---

## PRP を PRF として使う: Switching Lemma

ブロック暗号は PRP だが、モードの証明では PRF として扱えると便利。置き換えの誤差を定量化するのが **PRP/PRF Switching Lemma**:
...
`|X|` が大きければ（AES は `2^128`）現実的な `q` で差は無視でき、**安全な PRP はそのまま安全な PRF**。

**`|X|` が小さい反例**: `X = {0,1}` 上の `E(k,x) = x ⊕ k` は置換なので PRP。だが真のランダム関数 `f` は確率 1/2 で `f(0)=f(1)` になるのに、置換は決して衝突しない。「`f(0)=f(1)` か?」を調べる識別器で PRF としては即破れる。
...
決定的なので**同一の平文ブロックは同一の暗号文ブロック**になり、反復パターンが透ける（Linux Tux を ECB で暗号化すると輪郭が残る「ペンギン画像」）。

意味論的安全性は 1 メッセージでも崩れる。`m0 = (hello ∥ hello)`、`m1 = (hello ∥ world)` を提出すると:
...
第 1・第 2 ブロックが等しいかを見るだけで `b` が分かる。`Adv = 1`。

---

## 決定的カウンタモード(ワンタイム鍵用)

ブロック暗号を PRF として使い、パッドを作って XOR する:
...
| | ECB | 決定的カウンタモード |
|---|---|---|
| 式 | `c_i = E(k, m_i)` | `c = m ⊕ (F(k,0) ∥ F(k,1) ∥ …)` |
| 同一ブロックの露見 | あり | なし |
| 必要プリミティブ | PRP | **PRF で十分**（`E` の逆を使わない） |
| ワンタイム鍵での安全性 | ✗ | ○（証明はストリーム暗号と同型） |
| 同じ鍵で 2 メッセージ | ✗ | ✗ `c ⊕ c' = m ⊕ m'`（two-time pad） |
