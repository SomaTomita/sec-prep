...
```
素因数分解問題 (Integer Factorization Problem, IFP)
  n = p·q（p, q は大きな素数）が与えられたとき、p, q を求めよ。
  → RSA の安全性の根拠。最良の古典アルゴリズムは準指数時間（一般数体篩法 GNFS）。

離散対数問題 (Discrete Logarithm Problem, DLP)
  巡回群 G、生成元 g、y = g^x (x は秘密) が与えられたとき、x を求めよ。
  → DH 鍵共有・DSA・ElGamal の安全性の根拠。
  有限体 Z_p^* 上では最良の古典アルゴリズムはやはり準指数時間（指数計算法など）。

楕円曲線離散対数問題 (Elliptic Curve DLP, ECDLP)
  楕円曲線上の点がなす群で同様に x を求める問題。
  → ECDH・ECDSA の根拠。有限体上の DLP と違い、一般の楕円曲線には
    準指数時間アルゴリズムが知られていない。最良は Pollard の ρ 法で指数時間相当
    （群の位数の平方根程度）。同じ安全性を得るのに必要な鍵長が大幅に短くて済む
...
ところが **Shor のアルゴリズム**（1994年）は、量子コンピュータ上で

```
- 素因数分解 → 周期発見問題に帰着して多項式時間で解く
- 有限体上の DLP → 同じく多項式時間で解く
- ECDLP        → Shor の手法は一般の有限アーベル群上の DLP に一般化できるため、
                  楕円曲線の群も例外なく多項式時間で解けてしまう
```

を実現する。つまり「ECDLP は古典的には DLP より困難」という優位は、
...
[`../../../02_cryptography/`](../../../02_cryptography/00_index.md)（作成後は `00_index.md` から）で扱う。
