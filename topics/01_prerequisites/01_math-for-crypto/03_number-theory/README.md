...
「掛けるのは簡単、分解するのは困難」——素数の性質（フェルマー/オイラー・CRT・離散対数）が RSA・DH・ECC の「安全性の根拠」を作る。
[`../01_modular-arithmetic/`](../01_modular-arithmetic/README.md) の演算を土台に「なぜ正しく動くか」「なぜ安全か」を積み上げる。
