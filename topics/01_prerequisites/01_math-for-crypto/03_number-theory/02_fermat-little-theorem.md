...
「何を何乗しても、素数で割った余りは周期的に 1 に戻る」——この規則性がフェルマーの小定理。
逆元の別ルート・Miller-Rabin 素数判定・RSA の正当性証明がここから始まる。
...
```
p = 3:   2² = 4 ≡ 1 (mod 3)          a = 2, 指数 p−1 = 2
p = 5:   2⁴ = 16 ≡ 1,  3⁴ = 81 ≡ 1   どの a でも 4 乗で 1
```
...
7 の倍数でないどの `a` でも `a^6 ≡ 1 (mod 7)`。偶然ではなく、`p` が素数であることの必然。
...
```mermaid
flowchart TD
    F["フェルマーの小定理 a^(p-1) ≡ 1 (mod p)"] --> INV["逆元計算 a⁻¹ = a^(p-2) mod p"]
    F --> MR["Miller-Rabin 素数判定（合成数での不成立を検出）"]
    F --> EU["オイラーの定理へ一般化（合成数の法）→ RSA"]
```
...
合成数 `n` ではほとんどの `a` で `a^(n-1) ≡ 1 (mod n)` が**成り立たない**。この不成立を検出する確率的素数判定が **Miller-Rabin テスト**（Magma＝数式処理システムで実装する）。
