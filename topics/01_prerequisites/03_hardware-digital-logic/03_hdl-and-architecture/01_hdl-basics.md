...
回路をテキストで記述・シミュレート・合成するための言語で、Design Seminar の
...
この設計科目は Verilog/VHDL 両方を扱う可能性がある（シラバスに明記なし）。
...
flowchart LR
    ...
    N --> P[配置配線] --> T[タイミング解析] --> O["ビットストリーム（FPGA）<br/>or マスクデータ（ASIC）"]
```
...
- **FPGA**: 現場書き換え可能。プロトタイプ・教育用途に多用（セミナーで使用）
