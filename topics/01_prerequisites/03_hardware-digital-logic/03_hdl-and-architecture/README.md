...
回路を「コードで書く」言語が HDL。書いたものを合成ツールがゲートの網（ネットリスト）にし、FPGA/ASIC に焼く。
その上に積み上がる CPU の内部構造（パイプライン・メモリ階層）が、HW/SW 協調設計の共通言語になる。
3 ファイルは「部品を書く → 部品を並べて CPU にする → CPU が触るメモリと全体最適化」の一本道。
