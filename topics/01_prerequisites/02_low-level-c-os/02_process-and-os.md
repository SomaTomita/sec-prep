...
> 対応科目: H04K5A Development of Secure Software ｜ 前: [01 C言語とメモリ](./01_c-and-memory.md) ｜ 次: [03 システムソフトウェア](./03_systems-software.md) ｜ 層: 基礎(Layer 1)
...
- **setuid ビット**: 実行時にファイルオーナーの権限で動作する特殊フラグ
  ...
  - setuid バイナリの脆弱性は特権昇格に直結する（H04K5A「認証とアクセス制御」と接続）
...
H04K5A でカバーされる C 系脆弱性の「防御側」の話がここ。特に:
...
H0Q31A「セキュリティテスト（ペネトレーションテスト）」でも、これらの保護機構が  
