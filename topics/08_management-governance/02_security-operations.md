...
ここまでの議論は暗黙に「クラウドは1つ」を前提にしている。実際の組織は複数を併用しており、
**それは戦略的な選択というより、部門ごとの導入が積み重なった結果**であることが多い。

マルチクラウドで生じるのは、新種の攻撃ではなく**統制の分断**である。

| 分断する対象 | 何が起きるか |
|---|---|
| ID 管理 | 環境ごとに別の ID ストアができ、**退職者のアカウントが片方に残る** |
| ログ | 形式も保持期間も違い、**横断的な相関分析ができない** |
| ポリシー表現 | 「同じルール」を各環境の異なる記法で書き直すことになり、**差異が入り込む** |
| 鍵管理 | 環境ごとに別の鍵管理サービス。**どこにどの鍵があるか把握できなくなる** |
| 責任共有の境界 | サービスごとに境界が違い、**「誰が見るべき領域か」の認識が抜ける** |

**帰結**: 個々の環境が正しく設定されていても、**全体としての一貫性が保証されない**。
インシデント時に「どこを見ればよいか」が分からないことが最大の運用リスクになる。

責任共有モデル自体は
[`../06_systems-security/05_distributed-systems-security.md`](../06_systems-security/05_distributed-systems-security.md) が本体。
ここで押さえるのは**それが環境の数だけ存在するときの管理問題**。
第三国法の域外適用という別種のリスクは
[`../07_legal/03_regulatory-tsunami.md`](../07_legal/03_regulatory-tsunami.md) を参照。
...
4. ある組織が3つのクラウドを併用している。退職者のアクセス権を確実に剥奪するために、
   単一クラウドの場合と比べて追加で必要になることを2つ挙げよ。
...
4. 例: ①**3つの環境それぞれで剥奪を確認する手順**（ID ストアが分断しているため、
   1つの環境で無効化しても他に残る）②**環境をまたいだ棚卸しの仕組み**
   （誰がどの環境にアカウントを持つかの一元的な台帳がないと、そもそも
   漏れを検知できない）。他に、環境ごとに異なるログ形式を正規化して
   「剥奪後にアクセスがなかった」ことを横断的に確認する仕組みも挙げられる。
