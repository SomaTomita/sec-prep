...
- IAMの3要素（認証・認可・記録）とアクセス制御の設計原則
- BCPとDRの関係（DRはBCPの部分集合）と、復旧目標を測るRTO/RPO
- サードパーティ（サプライチェーン）リスク管理の骨格
...
```
Authentication（認証）: あなたは誰か？（パスワード・生体認証・MFA＝多要素認証等で確認）
...
|---|---|---|
| 目的 | 組織全体の「重要な事業機能」を災害発生中も継続させる | ITシステム・データを復旧させる |
| 範囲 | 代替オフィス・代替要員・代替業務プロセスまで含む広い概念 | BCPの一部——「事業を止めない」ためのIT面の技術的手段に限定 |
...
```mermaid
flowchart LR
    B["最後のバックアップ時点"] -->|"RPO: ここまでのデータ損失を許容"| F["障害発生"]
    F -->|"RTO: ここまでの停止時間を許容"| R["復旧完了"]
```
...
クラウド・SaaS（＝ネット経由で利用するソフトウェアサービス）・委託開発が一般化した結果、自組織が直接統制できない外部ベンダーの
