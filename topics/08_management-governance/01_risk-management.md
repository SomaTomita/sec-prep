...
「低減」（守りを厚くする）ばかりに目が行きがちだが、**そもそも守るべきものを作らない**
という選択がある。ベルギーの社会保障・医療情報の連携基盤は、この方針を国家規模で採った例である。

| 一般的な設計 | この事例の設計 |
|---|---|
| 各機関のデータを中央DBに集約する | **集約しない。** どの機関がどの情報を持つかの**参照情報だけ**を中央で管理する |
| 中央DBへのアクセス制御で守る | 各機関に対する**個別の照会**として権限を判定する |
| 侵害されると全データが漏れる | 中央が侵害されても**参照情報しか漏れない** |

**リスク管理としての含意**: データの集約は運用効率を上げるが、
**単一の侵害の影響を最大化する**。分散させると効率は落ちるが影響が限定される。
これは技術的な優劣ではなく、**どのリスクを受け入れるかの選択**である。

原則としてのデータ最小化は
[`../03_privacy/01_privacy-concepts.md`](../03_privacy/01_privacy-concepts.md)、
法的義務としての位置づけは
[`../03_privacy/06_law-and-dpia.md`](../03_privacy/06_law-and-dpia.md) を参照。
同じ「集約のリスク」はクラウドへの集中にも当てはまる
（→ [`../07_legal/03_regulatory-tsunami.md`](../07_legal/03_regulatory-tsunami.md)）。
