...
> 対応科目: H0Q27A, H0O37A ｜ 次: [02 運用管理・IAM・BCP/DR](./02_security-operations.md) ｜ 層: 基礎(Layer 1)
...
尤度 (Likelihood):     脅威が脆弱性を突く確率
影響度 (Impact):       実際に突かれた場合の被害の大きさ

リスク (Risk) ≈ 尤度 × 影響度
```

リスク対応の4選択肢: **低減**（対策を打つ）・**移転**（保険・アウトソース）・
**受容**（許容範囲として放置）・**回避**（そのリスクを生む活動自体をやめる）。
...
実務でどう回すかの詳細手順書。3段階のプロセスを定める:

```
1. 準備 (Prepare):   評価の目的・範囲・前提・制約を定める
2. 実施 (Conduct):    脅威源・脆弱性を洗い出し、尤度・影響度からリスクを算定する
3. 維持 (Maintain):   評価結果を監視し、環境変化に応じて更新する
```
...
```
NIST CSF/SP 800シリーズ: 無料公開・詳細な実装ガイド・米国政府機関で義務化されがち
ISO 27000シリーズ:      有償規格・認証ビジネスと結びつく・欧州企業の取引要件になりがち
```
