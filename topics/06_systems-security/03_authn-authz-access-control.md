...
```mermaid
flowchart LR
    S["主体<br>（ユーザー / プロセス）"] -->|アクセス要求| RM["参照モニタ"]
    M[("アクセス制御<br>マトリックス M")] -.->|照合| RM
    RM -->|権利あり: 許可| O["客体<br>（ファイル等）"]
    RM -->|権利なし: 拒否| X["拒否"]
```
...
分散システムのアクセストークン（OAuth 2.0＝Webサービス間で権限を委譲するための標準プロトコル、のトークン等）は後者の発想に近い。
...
| **MAC**（強制アクセス制御） | システムが定めた全体ポリシーに従う。所有者でも変更できない | Bell-LaPadula モデル、SELinux（LinuxにMACを追加するカーネル機構） |
