...
CIA は1970年代に整理された枠組みで、語彙としては今も有効だが、**現実の要求を表しきれない**
という批判がある。守りたいのは「機密性・完全性・可用性」という3つの状態ではなく、
契約の締結・公証・権限の委譲・データ分析からの推論のように、
**現実世界の複雑な相互作用をデジタルに移すこと**だという見方である。

例えば「あるデータ集合に分析をかけて得た結論」は、CIA のどれに属するか一意に決まらない。
元データは機密のままでも、結論から元データが推定できるなら何が守られたことになるのか——
これは CIA の語彙では書けない（→ [`../03_privacy/02_anonymization-and-dp.md`](../03_privacy/02_anonymization-and-dp.md)）。

**実務上の含意**: CIA を分類の出発点に使うのは妥当だが、
**「CIA のどれにも当てはまらないから要件ではない」と切り捨ててはいけない**。
関与する主体それぞれの権利を適切に守る、というより広い問いに置き直す必要がある。
