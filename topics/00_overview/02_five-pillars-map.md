...
- 柱同士がどのトピックで交わるか（横断トピックと本体の置き場ルール）
- 自分の前提知識がどの柱に効くか

セキュリティは 1 つの科目ではなく、守る対象の違う 5 つの分野の束。この地図で「どこで何が交わるか」だけ先に掴む。

## 最小の例：「パスワードでログイン」を 5 本柱で見る

| 柱 | この 1 機能で問うこと |
|---|---|
| 暗号 | パスワードをどうハッシュして保存するか |
| プライバシー | ログイン履歴から誰の行動が分かるか |
| HW | 鍵やハッシュ計算をどのチップで守るか |
| ソフトウェア | 入力欄に注入攻撃の穴がないか |
| システム | 認証後に何を許可するか（認可） |

たった 1 つの機能でも 5 本柱すべてが関わる。これが「独立ではない」の意味。
...
| 用語 | 一言で |
|---|---|
| TPM | PC 等に組み込むセキュリティチップ。鍵の保管や起動時の検証を担う |
| PUF | 製造ばらつきを「個体の指紋」として使う回路 |
| サイドチャネル | 消費電力や処理時間など**副次的な漏れ**から秘密を推定する攻撃 |
| MPC / FHE | データを秘匿したまま計算する暗号技術（秘密計算・完全準同型暗号） |
| GDPR | EU の個人データ保護規則。違反時の制裁金が大きい |
...
- 法務は 5 本柱の外にあるため図に含めない（GDPR ⇄ 法務は表を参照）
- AI は**全柱に横断的に**かかるため図に含めない（[04](04_threat-landscape.md) / [05](05_ai-and-security.md)）
