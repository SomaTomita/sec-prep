...
### 最小の例: TLS 1.3 を 3 メッセージで

```
1. ClientHello  クライアント → サーバ: 「この方式が使える」＋ 鍵共有の材料
2. ServerHello  サーバ → クライアント: 材料の返し ＋ 証明書 ＋ 署名
3. Finished     双方が同じ材料から鍵を作り、以降は暗号化通信
```

往復 1 回（1-RTT）で鍵が決まる。材料を交換するのが**鍵交換**、証明書と署名で相手を確かめるのが**認証**——
この 2 つを別々の道具で行うのが TLS 1.3 の骨格である。以下、その詳細と他プロトコルとの比較。
