...
暗号は「鍵という合鍵を持つ人だけが読める・書ける」仕組みを数学で作る道具箱。
先に「何を守りたいか（目標）」と「どの部品で守るか（プリミティブ）」の地図を持つ。CIA トライアド（機密性・完全性・可用性の 3 目標。[用語集](../00_overview/03_glossary.md)）のうち**暗号で守れないものがある**のが最初の要点。

## 最小の例: 「Bob へ 100 円送金」を Alice が送る

| 攻撃者 Eve がすること | 破られる目標 | 守る部品 |
|---|---|---|
| 途中で読む | 機密性 | 暗号化 |
| `100` を `900` に書き換える | 完全性 | MAC / 署名 |
| Alice のふりをして送る | 認証 | MAC / 署名 |
| Alice が後で「送っていない」と言う | 否認防止 | 署名のみ |
| 回線を溢れさせて届かなくする | 可用性 | **暗号では防げない** |

この 1 通のメッセージに起こりうる 5 つの事故が、以下の全節の目次になっている。
...
```mermaid
flowchart TD
  P["暗号プリミティブ"] --> U["無鍵: 鍵を使わない部品"]
  P --> S["対称鍵: 送受信者が同じ鍵"]
  P --> A["公開鍵: 公開鍵と秘密鍵のペア"]
  U --> U1["ハッシュ関数 SHA-256 / 乱数生成器"]
  S --> S1["AES / ChaCha20 / HMAC"]
  A --> A1["RSA / ECDSA / DH・ECDH"]
```

| 分類 | 速さ | 解く問題 | 詳述 |
|---|---|---|---|
| 無鍵 | — | 完全性の検出・乱数供給 | [04](./04_hash-and-mac.md) |
| 対称鍵 | 高速 | 大量データの機密性・認証 | [02](./02_symmetric-crypto/README.md) |
| 公開鍵 | 低速 | 鍵共有・署名（否認防止） | [03](./03_public-key-crypto/README.md) |
...
安全性は「攻撃者が何をできるか」を決めないと議論できない（攻撃者モデル）。代表的な分類:
