...
- CIA トライアドのうち暗号が守れる目標と守れない目標（可用性は暗号の範囲外）
- MAC では否認防止ができない理由と、デジタル署名が必要になる場面
- 無鍵・対称鍵・公開鍵という3分類のプリミティブ（＝暗号システムを組み立てる基本部品）の見取り図
...
どれだけ強い暗号を使っても、サーバを停止させる DoS 攻撃（＝リソースを枯渇させてサービスを妨害する攻撃）は防げない。
