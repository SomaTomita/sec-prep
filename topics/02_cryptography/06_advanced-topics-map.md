...
公開鍵暗号を使う機器は数百億台規模で存在し、**標準の完成は移行の始点にすぎない**。
難所には順序がある——通信の鍵共有（TLS 等）は比較的進めやすいが、**署名と PKI が難しい**
（証明書チェーン全体・組込機器の鍵長と検証コスト・更新機構自身が署名に依存する）。
プラットフォームとハードウェアの移行はさらに遅れる。

全数を一度に移すのは不可能なので、「今収集され将来復号される」危険
（**harvest now, decrypt later**）が高いデータから優先して移す。自組織が今どの暗号方式を
使っているかの棚卸し自体に年単位かかることも珍しくない。

**crypto agility**（後から方式を差し替えられる設計）は1990年代からの目標だが、
方式ごとに鍵長・署名長・往復回数・前提が異なるため、想定外の方式への差し替えは実際には難しい。
「アジリティがあるから大丈夫」と考えるのは危険である。
移行が規制上の要求になる文脈は [`../07_legal/03_regulatory-tsunami.md`](../07_legal/03_regulatory-tsunami.md)、
ハードウェア実装コストは [`../04_hardware-security/index.md`](../04_hardware-security/index.md) を参照。
