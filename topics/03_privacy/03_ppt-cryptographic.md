...
「暗号⇄プライバシー」の交差点として挙げた MPC・FHE の応用がここに当たる
（MPC・FHE 自体の仕組みは
...
単一サーバPIR: 1台のサーバに対して、準同型暗号（FHEやその軽量版）で暗号化した
...
- **Google Password Checkup**: 自分のパスワードのハッシュを「盲化」した値をサーバに送り、
  サーバは中身を知らないままその値に処理を施して返す（OPRF技術）。
