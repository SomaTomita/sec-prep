...
| **完全性・機密性** | 適切な技術的措置でデータを保護する | [`../02_cryptography/`](../02_cryptography/00_index.md) 全体が「技術的措置」の具体策 |
| アカウンタビリティ | 上記全てを遵守している**ことを証明できる**責任を管理者が負う | 本ファイルのDPIAがその手段の1つ |
...
以下のいずれかに該当する処理を行う前に、DPIAの実施が義務付けられる:
  1. 個人に法的・重大な影響を与える体系的・大規模な自動評価（プロファイリング等）
...
技術設計の観点からは **STRIDE/LINDDUN 脅威モデリング**
...
サイバー犯罪・知財等）は [`../07_legal/`](../07_legal/00_index.md)、DPIA実施時の
技術的脅威分析（LINDDUN）は
[`../05_software-security/02_threat-modeling.md`](../05_software-security/02_threat-modeling.md)
へ接続していく。
