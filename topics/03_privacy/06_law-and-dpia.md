...
### 最小の例: 社内の名簿を 1 つ扱うだけで

人事部が「氏名・メールアドレス・所属」の Excel を 1 つ持つ。これだけで GDPR は動き出す。
4 つの問いを順に当てるのが実務の型である。

| 問い | この例では | 根拠 |
|---|---|---|
| ① 個人データか | 氏名とメールで個人を識別できる → **該当** | 第4条 |
| ② 誰が管理者か | 目的と手段を決めたのは会社 → **会社が管理者** | 第4条 |
| ③ 適法根拠は | 雇用契約の履行に必要 → **契約の履行**（同意は不要） | 第6条 |
| ④ DPIA は要るか | 通常の名簿管理は高リスクでない → **不要** | 第35条 |

同じ名簿に「勤務中の位置情報を常時記録」を足すと ④ が **要る**に変わる。
以下、この 4 つの問いを支える骨格を順に見る。
