...
- 電力解析の3段階：SPA（波形を目視）→ DPA（大量波形の統計差分）→ CPA（漏洩モデルとの相関）
- キャッシュのヒット/ミスの時間差を悪用する Prime+Probe / Flush+Reload
- 2大対策：定数時間実装（秘密で時間・アクセス先を変えない）とマスキング（秘密を断片に分割）
...
CPA（相関電力解析、Brier, Clavier, Olivier, 2004年）: DPAの単純な差の代わりに、
  「消費電力はハミング重み（値のビット中の1の個数）／ハミング距離（ビット遷移数）に比例する」という
