...
暗号化したまま計算する FHE（[`../02_cryptography/06_advanced-topics-map.md`](../02_cryptography/06_advanced-topics-map.md)）は
理論的には十年以上前に実現されているが、実用化を阻んできたのは**性能**である。
専用ハードウェアが何を解こうとしているかの good example になる。

### 遅さの正体はノイズ

FHE の暗号文には安全性のため意図的に**ノイズ**が乗せてあり、**演算のたびに増える**
（特に乗算で急激に増える）。ノイズが閾値を超えると復号できなくなる。
...
**ブートストラップ**はノイズを減らして計算を続けられるようにする操作だが、
これ自体が極めて重い。**FHE の実行時間の大半がブートストラップに費やされる。**

### 高速化は1つの層では解けない

| 層 | 何を最適化するか |
|---|---|
| 専用ハードウェア | 多項式演算（NTT）と巨大な係数の並列処理、メモリ帯域 |
| ライブラリ | パラメータ選択、演算の実装 |
| **コンパイラ** | 平文プログラムを FHE 演算列に変換し、**どこでブートストラップを挟むか**を決める |

**コンパイラ層の判断が性能を大きく左右する**点が特徴的。乗算の順序を変えるだけで
必要なブートストラップ回数が変わるため、**アルゴリズムの書き方そのものが性能問題になる**。
どの場面で FHE を選ぶかは
